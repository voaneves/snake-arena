...
| ↳ **ACKTR — de onde vem o estouro da região de confiança** | — | `91_acktr_ablacoes.ipynb` | — | 🔬 A KL entregue é 4,4×–12,4× a pedida, e a revisão atribui isso à Fisher aproximada — que é a **premissa do ACEKTR**. Há dois suspeitos no mesmo lugar que não são a Fisher: o momento do SGD (o passo em regime é até 10× o que `escala_kl` calculou) e o `clipnorm` do PPO caindo sobre a direção já pré-condicionada. **Em medição.** Os braços sem `clipnorm` **saturam o teto de η** (`lr_start`) em até 83% das atualizações e não medem curvatura nenhuma — o `clipnorm` não é freio, é o que mantém a região de confiança operando. O braço limpo é `so_a_fisher` (sem momento, sem clip, sem teto). A célula de diagnóstico responde em ~2 min por braço |
...
| ACKTR — as ablações | [![Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/voaneves/snake-arena/blob/main/notebooks/91_acktr_ablacoes.ipynb) | [![Kaggle](https://kaggle.com/static/images/open-in-kaggle.svg)](https://kaggle.com/kernels/welcome?src=https://github.com/voaneves/snake-arena/blob/main/notebooks/91_acktr_ablacoes.ipynb) |
