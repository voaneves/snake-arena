...
NOTEBOOKS = [
    ...
    },
    {
        ...
        "resumo": "O `08_acktr` com o EK-FAC no lugar do K-FAC — e, desde 01/09, no "
                  ...
                  "20 pontos abaixo.\n\n"
                  "**A segunda tentativa também não valeu, e por outro motivo.** Em "
                  "02/09 o `inv_every` foi a 50 — o regime de amortização do paper, base "
                  "rara e escalas sempre. Fechou em 74,47 com **0,4%** de tabuleiros "
                  "cheios: média maior que a de 01/09 e taxa de vitória 44× menor. O "
                  "`ekfac_desvio` foi a **69,6** dentro da janela contra 0,2–0,4 com "
                  "janela de 10, e o pico escala com o comprimento da janela — ou seja "
                  "ele mede **base velha**, não violação de Kronecker. Com 610 "
                  "atualizações no orçamento inteiro, 50 delas são 8% do treino: a "
                  "premissa do paper não vale neste regime.\n\n"
                  "**O que este notebook roda hoje**, e é o braço oficial `acektr/"
                  "resnet_small` com três sementes (74,30 · 80,02 · 82,01): `rollout=16` "
                  "restaurado no `ACKTRConfig`; `inv_every=10`, base fresca, porque o "
                  "conserto da queixa do §3.2 é **estimador melhor** e não janela maior; "
                  "`escalas_acumuladas=True`, a média acumulada dentro da janela da base, "
                  "que é o estimador de mínima variância onde não há deriva a esquecer; e "
                  ...
                  "voltar.\n\n"
                  "**O padrão não ganha marca no nome.** Este notebook, sem tocar em "
                  "nada, grava `acektr/resnet_small`. Quem desvia é quem aparece: "
                  "`+s_ema` para a média móvel da implementação de referência, `+base50` "
                  "para o regime do paper, `+sem_correcao` para o controle que desliga a "
                  "medição e vira o K-FAC bit a bit. Ver `docs/EKFAC.md` §3.2, §3.3, §5.1 "
                  "e §5.2 — esta última é o par limpo, e a resposta que ele deu."},
    {
...
"braço, porque três coisas terem medição e as outras não é o resultado.\n\n"
    ...
    },
    {
        ...
        "resumo":
            ...
            "padrão é `bala_lr_menor`: a `bala_de_prata` mais a única correção que o "
            "ensaio dela pediu.\n\n"
            "---\n\n"
            "### Onde estamos\n\n"
